...
enum LispValue: CustomStringConvertible {
    ...
    case lambda(args: ASTNode, body: ASTNode)
    case noValue
    ...
    var number: Double? {
        ...
        case .boolean, .builtinFunc, .noValue, .lambda: return nil
        }
    }
    ...
    var boolean: Bool? {
        ...
        case .number, .builtinFunc, .noValue, .lambda: return nil
        }
    }
    ...
    var builtinFunc: LispFunction? {
        ...
        case .boolean, .number, .noValue, .lambda: return nil
        }
    }
    
    var lambda: (args: ASTNode, body: ASTNode)? {
        switch self {
        case .lambda(let args, let body): return (args, body)
        case .boolean, .number, .noValue, .builtinFunc: return nil
        }
    }
}
...
func lookup(symbolName: String, env: Environment) throws -> LispValue {
    if let value = env.values[symbolName] {
        return value
    } else {
        if let parent = env.parent {
            return try lookup(symbolName: symbolName, env: parent)
        } else {
...
func lisp_apply(oper: LispValue, operands: [LispValue], env: Environment) throws -> LispValue {
    switch oper {
    case .builtinFunc(let f):
        return try f(operands)
    case .lambda(let args, let body):
        guard let argsList = args.list else {
            // TODO: maybe make a specific error here
            throw EvalutorError.unexpectedArgumentType(.noValue)
        }
        guard argsList.count == operands.count else {
            // TODO: maybe make a specific error here
            throw EvalutorError.incorrectNumberOfArguments([])
        }
        var localEnv: Environment = .init(values: [:], parent: g_env)
        for (i, arg) in argsList.enumerated() {
            guard let argSymbol = arg.symbol else {
                // TODO: maybe make a specific error here
                throw EvalutorError.unexpectedArgumentType(.noValue)
            }
            localEnv.values[argSymbol] = operands[i]
        }
        return try lisp_eval(ast: body, env: localEnv)
    case .boolean, .number, .noValue:
        fatalError("TODO replace this with a thrown exception")
    }
}
...
func lisp_eval(ast: ASTNode, env: Environment) throws -> LispValue {
    if let value = ast.number {
        ...
        if head.symbol == "lambda" {
            guard let args = nodes[safe: 1] else {
                // TODO: maybe make a specific error here
                throw EvalutorError.incorrectNumberOfArguments([])
            }
            guard let body = nodes[safe: 2] else {
                // TODO: maybe make a specific error here
                throw EvalutorError.incorrectNumberOfArguments([])
            }
            ...
            return try lisp_apply(oper: oper, operands: operands, env: env)
        }
